...
== API
gioppler::set_thread_name(std::string_view thread_name);
gioppler::get_thread_id();
gioppler::create_request_id();
gioppler::hash_request_id(std::string_view request_id);
...
gioppler::contract::assert(bool condition);
gioppler::contract::Invariant invariant(lambda);
gioppler::contract::Ensure ensure(lambda);

gioppler::debug_message(std::string_view subsystem, std::string_view format_str, const Args&... args);
gioppler::set_breakpoint(std::string_view subsystem);

gioppler::Profile profile(std::string_view subsystem, std::string_view request_id = "", double workload = 0);

gioppler::register_test(std::string_view subsystem, std::string_view test_name, test);
gioppler::register_benchmark(std::string_view subsystem, std::string_view benchmark_name, benchmark);

// these are always enabled
gioppler::runtime_assert(bool condition);
gioppler::log_error(std::string_view format_str, const Args&... args);
gioppler::log_message(std::string_view format_str, const Args&... args);
