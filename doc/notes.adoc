...
gioppler::debug_message(std::string_view filter, StringFunction string_function);
gioppler::set_breakpoint(std::string_view filter);

gioppler::Profile profile(std::string_view filter, double workload = 0);

gioppler::register_test(std::string_view filter, std::string_view test_name, test);
gioppler::register_benchmark(std::string_view filter, std::string_view benchmark_name, benchmark);

// these are always enabled
gioppler::runtime_assert(bool condition);
gioppler::log_error(StringFunction string_function);
gioppler::log_branch(StringFunction string_function);
gioppler::log_message(StringFunction string_function);
